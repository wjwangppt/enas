...
from src.cnn.image_ops import drop_path
...
flags.DEFINE_string("child__fixed_arc", None, "")
flags.DEFINE_integer("cnn_micro_num_layers", 5, "")
#flags.DEFINE_integer("cnn_micro_out_filters", 48, "")
flags.DEFINE_integer("cnn_micro_num_cells", 6, "")

def _set_default_params(params):

  params.add_hparam('fixed_arc', FLAGS.child__fixed_arc)
  #params.add_hparam('child_out_filters', FLAGS.cnn_micro_out_filters)
  params.add_hparam('child_num_layers', FLAGS.cnn_micro_num_layers)
  params.add_hparam('child_num_cells', FLAGS.cnn_micro_num_cells)
  return params
...
class MicroChild(Model):
  def __init__(self,
    ...
    num_layers = self.params.child_num_layers

    num_cells = self.params.child_num_cells
      ...
      for conv_id in range(stack_conv):
        with tf.variable_scope("stack_{0}".format(conv_id)):
          ...
          with tf.variable_scope("bn"):
            ...
            one_init = tf.ones_initializer(dtype=tf.float32)
