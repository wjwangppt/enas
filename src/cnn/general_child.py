...
from src.cnn.models import Model
from src.cnn.image_ops import conv
from src.cnn.image_ops import fully_connected
from src.cnn.image_ops import batch_norm
from src.cnn.image_ops import batch_norm_with_mask
from src.cnn.image_ops import relu
from src.cnn.image_ops import max_pool
from src.cnn.image_ops import global_avg_pool
...
flags.DEFINE_string("child_fixed_arc", None, "")
flags.DEFINE_integer("cnn_macro_num_layers", 5, "")
#flags.DEFINE_integer("cnn_macro_out_filters", 48, "")

def _set_default_params(params):
  params.add_hparam('fixed_arc', FLAGS.child_fixed_arc)
# params.add_hparam('child_out_filters', FLAGS.cnn_macro_out_filters)
  params.add_hparam('child_num_layers', FLAGS.cnn_macro_num_layers)
  return params
...
class GeneralChild(Model):
  def __init__(self,
               ...
               cutout_size=None,
               num_branches=6,
               clip_mode="norm",      #don't delete
               grad_bound=5.0,        #don't delete

               lr_init=0.1,           #don't delete
               lr_dec_start=0,        #don't delete
               lr_dec_every=100,      #don't delete
               lr_dec_rate=0.1,       #don't delete
               lr_cosine=True,
               lr_max=0.05,
               lr_T_0=10,
               lr_T_mul=2,
               ...
               out_filters_scale=1,
               optim_algo="momentum", #don't delete
               sync_replicas=False,   #don't delete
               num_aggregate=None,    #don't delete
               num_replicas=1,        #don't delete
               data_format="NCHW",    #don't delete
               name="child",          #don't delete
               *args,
               **kwargs
              ):
    """
    ...
    whole_channels = self.params.controller_search_whole_channels
    fixed_arc = self.params.fixed_arc
    #out_filters_scale = self.params.child_out_filters_scale

    num_layers = self.params.child_num_layers             #Number of options in ENASs

    if (params.controller_training):
      lr_min=0.0005
    else:
      lr_min=0.001
  ...
  def _fixed_layer(
    ...
    if self.whole_channels:
      ...
      if count in [0, 1, 2, 3]:
        ...
        #pass
        out = tf.nn.avg_pool(inputs, ksize=[1, 3, 3, 1], strides=[1, 1, 1, 1], padding="SAME")
      elif count == 5:
        #pass
        out = tf.nn.max_pool(inputs, ksize=[1, 3, 3, 1], strides=[1, 1, 1, 1], padding="SAME")
      else:
